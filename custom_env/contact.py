...
def force_calc(angle, l_2):
    ...
    angles = [angle,10,20,30,40,50,60,70,80,90]
    ...
    deg_chunks = [adjusted_angle_list[x:x+10] for x in range(0, len(adjusted_angle_list), 10)]
    rad_chunks = [rad_angle_list[x:x+10] for x in range(0, len(rad_angle_list), 10)]
    ...
    area_chunks = [contact_area_list[x:x+10] for x in range(0, len(contact_area_list), 10)]
    ...
    return base_force_vector_list[0]
...
if __name__ == '__main__':
    ...
    base_force_vector_list  = force_calc(angle= 60, l_2=0.3)
